...
public class SessionCore<Job, In, IdFact, Id> : ISession<Job, In, IdFact, Id>
    ...
    where IdFact : IIdFactory<Id>
{
    ...
    // ctor
    SessionCore(Paths<Id, IdFact> paths, HashSet<string> jobResults)
    {
        Paths = paths;
    ...
    /// <returns></returns>
    public static Res<SessionCore<Job, In, IdFact, Id>> New(string rootDirectory, IdFact idFactory, HashSet<string> jobResults)
    {
        return
            Ok().Try(() =>
            {
            ...
            .Map(paths => new SessionCore<Job, In, IdFact, Id>(paths, jobResults));
    }
